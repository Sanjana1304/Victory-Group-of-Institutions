...
import InstrForm from './InstrForm';
...
const EnrolledStuds = ({inProgressStudents}) => {
  ...
  const [instAssignStudMail, setInstAssignStudMail] = useState('');
  const [instAssignCourseId, setInstAssignCourseId] = useState('');
  ...
  const handleInstAssign = (email,courseId) => {
    setInstAssignStudMail(email);
    setInstAssignCourseId(courseId);
  ...
  return (
    <div>
            ...
            {inProgressStudents.map((student,index) => (
              <tr key={index}>
                ...
                <td className="p-4 border-b">
                  {student.course.courseInstructor === 'To be assigned' ? 
                      <span className='text-red'>{student.course.courseInstructor}
                      <button className='mt-1 text-white bg-blue text-[8px] px-1' onClick={()=>handleInstAssign(student.email,student.course.courseId)}>Assign</button>
                      </span> 
                      : 
                      student.course.courseInstructor}
                </td>
                <td className="p-4 border-b">{student.course.coursePrice}</td>
                <td className="p-4 border-b">
                  {student.course.hasPaid?
                  <span className=' font-semibold'>Paid</span>
                  :
                  <span className='text-red font-semibold'>Pending<button className='mt-1 text-white bg-blue text-[8px] px-1' onClick={()=>handleMarkPaid(student.email,student.course.courseId)}>Mark Paid</button></span>}
                </td>
...

        {
          ...
          <Modal onClose={()=>setIsInstructorModalOpen(false)}>
            <InstrForm studentEmail={instAssignStudMail} courseId={instAssignCourseId} />
          </Modal>
        }
        </div>
    </div>
